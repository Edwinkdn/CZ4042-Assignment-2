...
def load_data(file):
    with open('/content/gdrive/My Drive/cz4042/{}'.format(file), 'rb') as fo:
        try:
...
def main():
    ...
    for map1 in range(10, 101, 10):
        for map2 in range(10, 101, 10):
    ...
    y = range(10, 101, 10)
    X, Y = np.meshgrid(x, y)
    test_acc_mean=test_acc_mean.reshape(10,10)
    plt.figure()
    ...
    plt.savefig('My Drive/cz4042/figures/Grid_Search_Accuracy')
