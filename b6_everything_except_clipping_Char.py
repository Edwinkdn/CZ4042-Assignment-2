...
def char_rnn_model(x, DROPOUT, control_variable):
  ...
  if control_variable==3:
    cell = tf.contrib.rnn.MultiRNNCell([create_cell(1) for _ in range(2)])
  else:
    cell=create_cell(control_variable)
    ...
    with tf.Session() as sess:
        ...
        for e in range(no_epochs):
            ...
            for start, end in zip(range(0, N, batch_size), range(batch_size, N, batch_size)):
                char_list_, _, loss_ = sess.run([char_list, train_op, entropy], {x: x_train[start:end], y_: y_train[start:end]})
